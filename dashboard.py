...
st.markdown(
    """
    This dashboard analyzes cryptocurrency market risk using historical price volatility
    and recent risk trends. Users can explore risk metrics for individual cryptocurrencies
    through an interactive selector.
    """
)
...
coin_file = f"data/{selected_coin.lower()}_price_30_days.csv"

if os.path.exists(coin_file):
    hist_df = pd.read_csv(coin_file)
    ...
    st.warning("Historical data file not found for the selected coin.")
...
for coin in df["Coin"]:
    file_path = f"data/{coin.lower()}_price_30_days.csv"
